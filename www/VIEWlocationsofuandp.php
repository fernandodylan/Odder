...
<!--<img src="/images/orca.jpg" alt="A whale used as a test image (Are you surprised that Zane would do this?!)">-->
        <section id="content">
          ...
          <p> Odder has satisfied users across the globe! Here is a complete list of countries Odder has created jobs in.</p>
          <br>
          <?php
          ...
          try {
          	$dbh = new PDO("mysql:host=$hostname;dbname=odder_database",$username,$password);
          	$dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); // <== add this line
            $sql = $dbh->prepare("SELECT * FROM locandpost");
            $sql->execute();
?>
